...
export default function PublicSite() {
  ...
  return <div className={`min-h-screen overflow-x-hidden bg-[#fbfcfa] text-[#263e48] site-${template.layout.toLowerCase()}`} style={style}>
    <header className="relative z-30 bg-white">
      ...
      <nav className="flex gap-2 overflow-x-auto border-t border-[#e8eeeb] px-4 py-3 lg:hidden" aria-label="모바일 주 메뉴">{roots.map((root) => <Link className="shrink-0 rounded-full bg-[#eef7f6] px-4 py-2 text-[0.6875rem] font-bold no-underline" key={root.id} to={root.path}>{root.name}</Link>)}</nav>
    </header>
...
function Home({ template, notices }: { template: SiteTemplate; notices: Post[] }) {
  return <main>
    ...
    <section className="mx-auto grid max-w-[77.5rem] gap-10 border-t border-[#e4ece9] px-5 pb-[4.375rem] pt-[1.875rem] lg:grid-cols-[.7fr_1.3fr]"><div><p className="text-[0.625rem] font-bold tracking-[.14em] text-[var(--brand)]">NEWS &amp; NOTICE</p><h2 className="my-[0.4375rem] text-[1.5625rem] tracking-[-.05em]">새로운 소식</h2><p className="text-[0.8125rem] leading-[1.8] text-[#849597]">AX Bio Studio의 주요 소식과 안내를 확인하세요.</p><Link className="mt-4 inline-block text-[0.6875rem] font-bold text-[var(--brand)]" to="/support/notices">공지사항 전체보기 →</Link></div><div className="border-t border-[#e4ece9]">{notices.slice(0, 4).map((notice) => <Link className="grid gap-2 border-b border-[#e4ece9] py-[1.125rem] text-inherit no-underline md:grid-cols-[1fr_auto]" key={notice.id} to={`/posts/${notice.id}`}><strong className="text-[0.8125rem]">{notice.title}</strong><span className="text-[0.6875rem] text-[#9aa8a9]">{date(notice.createdAt)}</span></Link>)}</div></section>
  </main>
}
