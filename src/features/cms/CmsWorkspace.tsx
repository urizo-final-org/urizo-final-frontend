...
export default function CmsWorkspace({ route, api }: { route: RouteId; api: CmsApi }) {
  ...
  return <>
    ...
    {assistantRoute
      ? <div className={`grid items-start gap-5 ${assistantCollapsed ? 'min-[1180px]:grid-cols-[minmax(0,1fr)_72px]' : 'min-[1180px]:grid-cols-[minmax(0,1fr)_350px]'}`}>
...
function SuccessToast({ notice }: { notice: SuccessNotice | null }) {
  return notice ? <div className="pointer-events-none fixed inset-0 z-[100] grid place-items-center p-5" aria-live="polite" aria-atomic="true">
    <div key={notice.id} className="cms-success-toast flex max-w-[520px] items-center gap-3 rounded-2xl bg-[#17241f] px-6 py-5 text-sm font-bold text-white shadow-[0_24px_70px_rgba(10,20,17,.35)]" role="status"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#1bb889] text-base" aria-hidden="true">✓</span><span>{notice.message}</span></div>
  </div> : null
}
function Feedback({ failure }: { failure: string | null }) { return failure ? <p className="mb-5 rounded-lg border border-[#efc7cc] bg-[#fff7f8] p-3 text-sm text-red" role="alert">{failure}</p> : null }
...
function Templates({ api }: { api: CmsApi }) {
  ...
  return <>
    ...
    <div className="mb-5 grid gap-3 md:grid-cols-3">
      {items.map((item) => <article key={item.key} className={`${card} grid gap-4 ${value?.key === item.key ? 'border-purple ring-2 ring-[#ded9ff]' : ''}`}>
        <button type="button" className="text-left" aria-label={`${item.key} 템플릿 선택`} onClick={() => select(item)}>
          <span className="text-xs font-bold text-purple">{item.key}{item.active ? ' · ACTIVE' : ''}</span>
          <h2 className="mb-1 mt-3">{item.siteName}</h2>
          <span className="text-xs text-muted">{item.layout}</span>
        </button>
    ...
    {value && <form className={`${card} grid gap-5 md:grid-cols-2`} onSubmit={submit}>
      <div className="flex items-center gap-3 md:col-span-2">
        <div className="mr-auto"><strong className="block text-sm">{value.key} 템플릿 설정</strong><span className="text-xs text-muted">저장하면 이 템플릿이 사용자 사이트에 적용됩니다.</span></div>
...
function TemplateField({ label, description, wide = false, children }: { label: string; description: string; wide?: boolean; children: ReactNode }) {
  return <label className={`grid content-start gap-2 text-xs font-bold ${wide ? 'md:col-span-2' : ''}`}><span>{label}</span>{children}<span className="font-normal leading-5 text-muted">{description}</span></label>
}
...
function TemplatePreview({ value, onClose }: { value: SiteTemplate; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[#0e1726]/70 p-4" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose() }}>
    <section className="max-h-[92vh] w-[min(980px,96vw)] overflow-auto rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label={`${value.key} 템플릿 미리보기`} style={style}>
      <div className="sticky top-0 z-10 flex items-center border-b border-line bg-white px-5 py-4"><div className="mr-auto"><strong className="block text-sm">{value.key} 템플릿 미리보기</strong><span className="text-xs text-muted">저장 전 화면 구성 예시입니다.</span></div><button type="button" className={secondary} onClick={onClose}>닫기</button></div>
